...
namespace CityLibraryInfrastructure.Extensions
{
    public static class GenerateAndVerifyPasswords
    {
        ...
        {
            if (password == null)
                throw new CustomException(Localizer["Password_Null"]);
            if (string.IsNullOrWhiteSpace(password))
                throw new CustomException(Localizer["Password_Space"]);
            if (password.Length < MinimumPasswordLength)
                throw new CustomException( string.Format(Localizer["Password_Min_Length"], MinimumPasswordLength));
